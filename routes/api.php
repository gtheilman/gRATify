...
Route::middleware('api')->group(function () {
    ...
    Route::get('/migration-status', [MigrationStatusController::class, 'publicStatus'])
        ->name('migrations.public-status');
});
...
Route::middleware(['api', 'auth:web'])->group(function () {
    ...
    Route::get('/shortlink-providers', function () {
        ...
        $cacheKey = "shortlink-providers.{$userId}";
        $cached = cache()->get($cacheKey);
        if ($cached) {
            return response()->json($cached);
        }
